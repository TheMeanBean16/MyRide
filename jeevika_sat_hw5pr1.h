...
istream& operator>>(istream& is, Place_info& place) {
	...
	double lat;
	double lon;								//location (lat and lon)
	String photo;                          //file location of the photo

	is >> name >> address >> lat >> lon;    //read in variables
	...
	//initialize place
	place = Place_info(name, address, tags, Geo_loc(lat,lon), photo);
...
public:
	...
	void addPlace(const Place_info &p)
	{
		locations.push_back(p);       //adds place to the system

									//confirms to the user
		cout << endl << p.getName() << " has been added to the system.\n" << endl;
	}

	bool removePlace(const Place_info &p) {

		for (int i = 0; i < locations.size(); i++) {
			if (locations[i].getName() == p.getName()) {
	...
	bool makePayment(string name, double payment) {
		...
		if (customers.size() == 0) {
			...
			return false;
				...
				//input is valid
				int customer_index = -1;      //customer hasnt been found yet
