...
add_place(
          Point(200, 300), 150, 75, "Add Place", cb_add_place),
...
p_longitude(Point(200, 220), 150, 50, "Place longitude: "),
...
void Lines_window::Add_Place() {
	...
	ss << p_name.get_string() << " "
		<< p_latitude.get_string() << " "
		<< p_longitude.get_string() << " "
		<< p_tags.get_string() << " "
		<< p_photo_name.get_string() << " ";
	...
	if (!ss) {
		my_error("Invalid place information");
	}
